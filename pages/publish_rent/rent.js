...
Page({
  data: {
    ...
    house_msg: {
      ...
      decorate: [],//装修
      decorate_index: [],//装修下标
      identity: [{ 'id': 1, 'name': '经济人' }, { 'id': 2, 'name': '个人' }],
      identity_index: 0,
    ...
    tab_bar: [{ 'name': '租房', 'selected': 1, 'type': 3 }, { 'name': '商铺', 'selected': 0, 'type': 4 }],//选项卡
    lispic: [],//概况图片列表
  ...
  build() {//建筑类型列表，picker使用
    let that = this;
    app.wxRequest('/Wxsite/Myuser/apiN', { 'api_name': 'houseType','type':2}, function (res) {
      console.log(res);
      if (res.data.code == 1) {
  ...
  formSubmit(e) {//表单提交
    ...
    }else if (app.tools.isNull(res.price)) {
      wx.showToast({
        icon: 'loading',
        title: (that.data.type == 3 || that.data.type == 4) ? '请输入租金' : '请输入单价'
      });
    ...
    // }
      ...
      wx.showModal({
        ...
        success: function (sm) {
          ...
          wx.uploadFile({
            url: 'https://jk.0759hn.com//Wxsite/Myuser/api',
        ...
        //   app.wxRequest('https://jk.0759hn.com/Wxsite/Myuser/api', data, function (res) {
        //     if (res.data.code == 1) {
        //       wx.showToast({
        //         title: '保存成功',
        //       });
