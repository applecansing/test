...
Page({
  data: {
    ...
    msg: [],
    tab_bar: [{ 'name': '二手房', 'selected': 1 }, { 'name': '新房', 'selected': 0 }, { 'name': '商铺', 'selected': 0 }],//选项卡
    lispic: [],//概况图片列表
  ...
  get_details() {
    ...
    app.wxRequest('/Wxsite/Myuser/api', { 'api_name': 'housesInfo', 'token': app.globalData.token, 'id': this.data.id }, function (res) {
      console.log(res);
      wx.hideLoading();
      if (res.data.code == 1) {
        ...
        let house_msg = that.data.house_msg;
        house_msg.area.forEach((item, index) => {
          if (item.area_id == msg.area_id) {
            house_msg.area_index = index;
            return;
          }
        });
        house_msg.build.forEach((item, index) => {
          if (item.id == msg.house_type_id) {
            house_msg.build_index = index;
            return;
          }
        });
        house_msg.decorate.forEach((item, index) => {
          if (item.title == msg.name) {
            house_msg.decorate_index = index;
            return;
          }
        });
        let floor = msg.floor;
        house_msg.floor_index[0] = floor.split('共')[0].replace(/[^0-9]/ig, "") - 1;//楼层后台过来的是字符串，提取数字picker使用
        house_msg.floor_index[1] = floor.split('共')[1].replace(/[^0-9]/ig, "") - 1;//楼层后台过来的是字符串，提取数字picker使用
        house_msg.house_type_index[0] = msg.layout_s - 1;
        house_msg.house_type_index[1] = msg.layout_t;
        house_msg.house_type_index[2] = msg.layout_c;
        house_msg.state_index = msg.house_status - 1;
        house_msg.lat = msg.lat;
        house_msg.lng = msg.lng;
        house_msg.identity_index = msg.is_type - 1;
        that.setData({
          msg: msg,
          house_msg: house_msg,
          indexpic: msg.indexpic,//主图
          indexpic_path: msg.indexpic,//主图提交后台地址
          lispic: msg.lispic,//房源概况图片
          lispic_path: msg.lispic//房源概况图片提交后台地址
        });
      } else {
        wx.showToast({
          icon: 'loading',
  ...
  build() {//建筑类型列表，picker使用
    let that = this;
    var typeValue = that.data.type == 1 || that.data.type == 2 || that.data.type == 5 ? 1 : 2;
    console.log(typeValue)
    app.wxRequest('/Wxsite/Myuser/apiN', { 'api_name': 'houseType', 'type': typeValue }, function (res) {
      console.log(res);
      if (res.data.code == 1) {
  ...
  formSubmit(e) {//表单提交
    ...
    } else if ((that.data.type == 3 || that.data.type == 4) && app.tools.isNull(res.price)) {
      wx.showToast({
        icon: 'loading',
        title: '请输入租金'
      });
    } else if (app.tools.isNull(res.room) || res.room == '0,0,0') {
      wx.showToast({
        icon: 'loading',
        title: '请选择房型'
      });
    ...
    // }
      ...
      let data = {
        ...
        'lispic': that.data.lispic_path ? that.data.lispic_path.join(',') : '',
        'lat': res.lat,
        'lng': res.lng,
        'room': res.room,
